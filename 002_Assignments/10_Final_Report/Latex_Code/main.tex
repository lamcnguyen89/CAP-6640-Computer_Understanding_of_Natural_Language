...
Due to time restraints, I was not able to build a user interface to interact with the LLM model and ask questions about the logs. However, I was able to implement the backend code to retrieve logs, convert them to embeddings and store them in a vector database. I was also able to implement the backend code to analyze the logs using the RAG pattern and the ChatGPT-4 model. The system was able to catch anomalies such as errors and excessive logins. However, due to limits on time and the cost of using the API, the project was not as refined as it could be. The Chat User interface for asking questions about the logs and getting insights was not fully implemented and there were still issues with implementing the system to run smoothly. However, the project was able to demonstrate the feasibility of using LLMs for log analysis.
