...
vocab_size = 1000 embed_dim = 50 embedding = nn.Embedding(vocab_size, embed_dim)
sentence = torch.tensor([12, 45, 332, 78]) embedded = embedding(sentence) embedded.shape</ pre>
Think about:
...
Define a 1D convolution over embedded text
Apply filters with different kernel widths
conv = nn.Conv1d(in_channels=embed_dim, out_channels=100, kernel_size=3)
x = embedded.transpose(0,1).unsqueeze(0) features = conv(x) features.shape
Think about:
...
pooled =
torch.max(features, dim=2).values pooled.shape
...
1
spm.SentencePieceTrainer.train( input='toy_corpus.txt', model_prefix='spm', vocab_size=200,
model_type='bpe' )
sp = spm.SentencePieceProcessor() sp.load('spm.model') sp.encode("unbelievable", out_type=str)</ pre>
Think about:
